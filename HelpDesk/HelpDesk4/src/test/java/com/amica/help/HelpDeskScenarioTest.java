...
public class HelpDeskScenarioTest {
	...
	public static void assertTrue(boolean condition, String error) {
		assertThat(String.format("    ASSERTION FAILED: %s", error), condition, equalTo(true));
	}
	
	public static void assertEqual(Object actual, Object expected, String error) {
		assertThat(String.format("    ASSERTION FAILED: %s %s", error, actual), actual, equalTo(expected));
	}
	
	public static void assertCount(Stream<?> results, long expectedCount, String error) {
	...
	/**
	 * This method tests that the implementation meets the following requirements.
	...
	public void test1_Tickets() {
		...
		assertCount(helpDesk.getTicketsByStatus(Status.WAITING), 2,
