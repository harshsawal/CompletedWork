...
public class Reporter {
	...
	public void printToFile(String path, String header, Stream<Invoice> data) {
		...
		try(PrintWriter output = new PrintWriter(new FileWriter(path))){
			...
			output.println("----  ------------------------  ----------  ----------  ----------");
			data.toList().forEach(s -> output.println(String.format("%4s  %-24s  %10s  %10s %10s", s.getNumber(),s.getCustomer().getName(),s.getInvoiceDate(),s.getAmount(),s.getPaidDate().orElse(null))));
			//data.toList().forEach(s -> output.println(" "+s.getNumber()+"  "+s.getCustomer().getName()+" "+s.getInvoiceDate()+" "+s.getAmount()+" "+s.getPaidDate()));
			//data.forEach(s -> output.println(s));
			output.close();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
